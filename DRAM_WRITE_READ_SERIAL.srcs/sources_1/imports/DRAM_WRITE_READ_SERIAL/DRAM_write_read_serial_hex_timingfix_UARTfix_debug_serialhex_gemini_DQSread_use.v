...
IOBUFDS #(
    ...
    if (delay_cnt == 0) begin
        ...
        odt <= 1;
        dqs_capture_en <= 0;
        captured_read_data <= 16'hDEAD;  // Clear old data
    end
    // Enable DQS capture window around CL timing
    else if (delay_cnt == 16) begin  // A bit before CL=9 (18 half-cycles)
        dqs_capture_en <= 1;
        nop();
    end
    else if (delay_cnt == 24) begin  // After data burst complete
        dqs_capture_en <= 0;
        odt <= 0;
        // Use either posedge or negedge captured data
        // You may need to experiment which gives correct data
        captured_read_data <= captured_read_data_posedge;
        // captured_read_data <= captured_read_data_negedge;  // Try this if posedge doesn't work
        nop();
    end
