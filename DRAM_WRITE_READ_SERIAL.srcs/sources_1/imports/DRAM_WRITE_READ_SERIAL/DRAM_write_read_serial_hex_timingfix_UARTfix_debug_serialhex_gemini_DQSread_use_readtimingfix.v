...
module dram_simple (
	...
	localparam CL_VAL = 9;
        localparam NUM_DDR_CLK_BURST_CYCLES = 4; // For BL8, data is transferred over 4 full DDR clock cycles
...
IOBUFDS #(
    ...
    always @(posedge clk_ddr) begin
        ...
        case (state)
          ...
          READ: begin
                // Define CAS Latency and Burst Duration in DDR clock cycles
                // Your MR0 (14'b00010100110000) sets CL=9 (A6,A4,A2=100) and BL8 (A1,A0=00)
                //localparam CL_VAL = 9;
                //localparam NUM_DDR_CLK_BURST_CYCLES = 4; // For BL8, data is transferred over 4 full DDR clock cycles
                ...
                if (delay_cnt == 0) begin
                    ...
                    odt <= 1;
                    dqs_capture_en <= 0;
                    // Initialize the registers that are directly updated by DQS edges
                    // This helps confirm if they get overwritten by actual captured data.
                    captured_read_data_posedge <= 16'hFACE; // Use a distinct pattern
                    captured_read_data_negedge <= 16'hBEEF; // Use a distinct pattern
                    // captured_read_data (the final output) will be updated after the window
                end
                // Enable DQS capture window: Start 1 cycle before expected data (CL_VAL - 1)
                // Data is expected to start arriving CL_VAL cycles after read_cmd.
                else if (delay_cnt == (CL_VAL - 1)) begin // e.g., for CL=9, this is at delay_cnt = 8
                    dqs_capture_en <= 1;
                    nop();
                end
                // Disable DQS capture window and latch the captured data.
                // Window should be active for NUM_DDR_CLK_BURST_CYCLES.
                // If enabled at (CL_VAL - 1), it should be disabled at (CL_VAL - 1) + NUM_DDR_CLK_BURST_CYCLES + 1 (to include the last cycle).
                // Example: enable at 8. For 4 burst cycles (8,9,10,11), disable at 8+4 = 12.
                // Let's make the window cover (CL_VAL-1) through (CL_VAL-1) + NUM_DDR_CLK_BURST_CYCLES. Disable on the next cycle.
                else if (delay_cnt == ( (CL_VAL - 1) + NUM_DDR_CLK_BURST_CYCLES + 1) ) begin // e.g., (8 + 4 + 1) = 13
                    dqs_capture_en <= 0;
                    odt <= 0;
                    // Latch the data captured on the posedge of DQS.
                    // This assumes the first word of the burst is desired.
                    // The values in captured_read_data_posedge should have been updated by DQS edges
                    // during the dqs_capture_en window if DQS toggled and data was present.
                    captured_read_data <= captured_read_data_posedge;
                    nop();
                end
                // Signal read completion a couple of cycles after latching internal data
                else if (delay_cnt == ( (CL_VAL - 1) + NUM_DDR_CLK_BURST_CYCLES + 1 + 2) ) begin // e.g., 13 + 2 = 15
                    read_data_ddr <= captured_read_data; // captured_read_data now holds the latched value
                    read_complete_ddr <= 1;
                    nop();
                end
                // Transition to PRECHARGE after sufficient delay
                // Ensure this is long enough for tRAS (if no auto-precharge) and other recovery.
                // Original was > 30. Let's adjust relative to our new timing.
                else if (delay_cnt > ( (CL_VAL - 1) + NUM_DDR_CLK_BURST_CYCLES + 1 + 2 + 10) ) begin // e.g., > (15 + 10) = 25
                    state <= PRECHARGE;
                    delay_cnt <= 0;
                end
                else begin
