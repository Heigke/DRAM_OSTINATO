...
//=================================================================
//  Mini UART-to-AXI-Lite bridge  (fixed enable bit + full T output)
//=================================================================
`timescale 1ns/1ps
module mini_uart_axi #(
    ...
    // ----------  Timing controls  ----------
    output reg         cfg_timing_en,
    output reg  [3:0]  cfg_twr_trim,
    output reg  [3:0]  cfg_tras_trim,
    output reg  [3:0]  cfg_write_cycles,
    // ----------  Debug ----------
...
// ────────────────────────────────────────────────────────────────
//  Power-up delay so DDR initialises before first access
// ────────────────────────────────────────────────────────────────
reg [26:0] init_cnt = 0;
...
reg [3:0]  nib_cnt;
reg        wr_tag, rd_tag;
reg [31:0] wr_addr, wr_data, rd_addr;
reg        cmd_is_write;
...
always @(posedge clk_i) if (rst_i) begin
    ...
    cfg_twr_trim    <= 4'd0;
    cfg_tras_trim   <= 4'd0;
    cfg_write_cycles<= 4'd8;   // default burst = 8
end else begin
    // clear “work to do” flags once the AXI sub-FSMs see them
    if (wr_done) wr_tag<=0;
    if (rd_done) rd_tag<=0;
    ...
    P_IDLE: begin
        ...
        else if(rx_data=="C"||rx_data=="c") begin
            p_state<=P_CONFIG;
        end
    end
    // ────────────────────  address field  ────────────────────────────
    P_GETADR: begin
        if(is_hex) begin
            ...
            wr_addr <= shift[31:0];  shift<=0; nib_cnt<=0; p_state<=P_GETDAT;
        end else if (rx_data==8'h0d||rx_data==8'h0a) begin
            if (!cmd_is_write && init_done) begin
                rd_addr <= shift[31:0]; rd_tag <= 1;
            end
            p_state<=P_IDLE;
        end else if(!is_hex && rx_data!=" ") p_state<=P_IDLE;
    end
    // ────────────────────  data field (write)  ───────────────────────
    P_GETDAT: begin
        if(is_hex) begin
            ...
            if(init_done) begin wr_data<=shift[31:0]; wr_tag<=1; end
            p_state<=P_IDLE;
        end else if(!is_hex) p_state<=P_IDLE;
    end
    // ────────────────────  timing config  ────────────────────────────
    P_CONFIG: begin
        if(is_hex) begin
            shift<={shift[59:0],hex2n(rx_data)}; nib_cnt<=nib_cnt+1;
        end else if (rx_data==8'h0d||rx_data==8'h0a) begin
            case (nib_cnt)
            // legacy 2-nibble mode  C<en><cycles>
            2: begin
                cfg_timing_en    <= shift[4];
                cfg_write_cycles <= (shift[3:0]==4'd0)?4'd8:shift[3:0];
                cfg_twr_trim<=4'd0; cfg_tras_trim<=4'd0;
            end
            // new 4-nibble mode  C<trAS><tWR><enable><cycles>
            4: begin
                cfg_tras_trim    <= shift[15:12];
                cfg_twr_trim     <= shift[11:8];
                cfg_timing_en    <= shift[4];              // bit0 of 3rd nibble
                cfg_write_cycles <= (shift[3:0]==4'd0)?4'd8:shift[3:0];
            end
            // “C0” disables everything
            1: if(shift[3:0]==0) begin
                cfg_timing_en<=0; cfg_twr_trim<=0; cfg_tras_trim<=0; cfg_write_cycles<=4'd8;
            end
            endcase
            p_state<=P_IDLE;
...
// ────────────────────────────────────────────────────────────────
//  AXI write interface  (unchanged)
// ────────────────────────────────────────────────────────────────
localparam A_IDLE=2'd0,A_AW=2'd1,A_W=2'd2,A_B=2'd3;
reg [1:0] astate=A_IDLE;
assign wr_done = (astate==A_IDLE)&&wr_tag;
always @(posedge clk_i) begin
    if(rst_i) begin awvalid_o<=0; wvalid_o<=0; wlast_o<=0; astate<=A_IDLE; end
    else case(astate)
    A_IDLE:if(wr_tag)begin
        awaddr_o<=wr_addr; wdata_o<=wr_data;
        awvalid_o<=1; wvalid_o<=1; wlast_o<=1; astate<=A_AW; end
    A_AW: begin
        if(awready_i) awvalid_o<=0;
        if(wready_i)  begin wvalid_o<=0; wlast_o<=0; end
        if(!awvalid_o&&!wvalid_o) astate<=A_B; end
    A_B:  if(bvalid_i) astate<=A_IDLE;
    endcase
end

// ────────────────────────────────────────────────────────────────
//  AXI read interface  (unchanged)
// ────────────────────────────────────────────────────────────────
localparam R_IDLE=2'd0,R_AR=2'd1,R_DATA=2'd2;
reg [1:0] rstate=R_IDLE;
assign rd_done = (rstate==R_IDLE)&&rd_tag;
always @(posedge clk_i) begin
    if(rst_i) begin arvalid_o<=0; rstate<=R_IDLE; end
    else case(rstate)
    R_IDLE: if(rd_tag)begin araddr_o<=rd_addr; arvalid_o<=1; rstate<=R_AR; end
    R_AR:   if(arready_i) begin arvalid_o<=0; rstate<=R_DATA; end
    R_DATA: if(rvalid_i && rlast_i) rstate<=R_IDLE;
    endcase
end

assign wstrb_o=4'hF; assign awid_o=4'd0; assign arid_o=4'd0;
assign bready_o=1'b1; assign rready_o=1'b1;

// ────────────────────────────────────────────────────────────────
//  UART transmitter – status / timing echo
// ────────────────────────────────────────────────────────────────
function [7:0] n2h(input [3:0] n); n2h=(n<10)?(n+"0"):(n-10+"a"); endfunction

// a tiny 2-state FSM so “T” prints *all* five bytes
reg [1:0] t_state;          // 0-idle  1-arm  2-printing
always @(posedge clk_i) begin
    case(t_state)
    2'd0: if(rx_stb && rx_data=="T") t_state<=2'd1;
    2'd1: if(!tx_busy)               t_state<=2'd2;   // wait for UART idle
    2'd2: if(!tx_busy && tx_cnt==4'd15) t_state<=2'd0;// newline done
    endcase
end
wire do_timing = (t_state==2'd2);

// one-shot for board status “?”
reg s_state;
always @(posedge clk_i)
    if(rx_stb && rx_data=="?") s_state<=1;
    else if(!tx_busy && s_state) s_state<=0;
wire do_status=s_state;

reg [3:0] tx_cnt=4'd15, tx_busy_d;
always @(posedge clk_i) begin
    tx_stb<=0; tx_busy_d<=tx_busy;
    // Timing message “E/D + tRAS + tWR + cycles + LF”
    if(do_timing && !tx_busy) begin
        case(tx_cnt)
    ...
    // Board status “R/W + LF”
    else if(do_status && !tx_busy) begin
        case(tx_cnt)
    ...
    // Read-data hex echo (unchanged)
    else if(rvalid_i && !tx_busy)        tx_cnt<=0;
    else if(!tx_busy && tx_busy_d && tx_cnt<4'd15) tx_cnt<=tx_cnt+1;

    if(!do_status && !do_timing && !tx_busy && tx_cnt<=8) begin
        case(tx_cnt)
...
always @(posedge clk_i)
    if(rst_i) debug_config_changed_reg<=0;
    else if(p_state==P_CONFIG && rx_stb && (rx_data==8'h0d||rx_data==8'h0a))
            debug_config_changed_reg<=1;
    else      debug_config_changed_reg<=0;
