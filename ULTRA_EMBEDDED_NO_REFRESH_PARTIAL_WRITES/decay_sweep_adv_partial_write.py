...
class DDR3Tester:
    def __init__(self, serial_port):
        ...
        # Wait for DDR3 initialization
        self.initialize_system()
    
    def initialize_system(self):
        """Initialize DDR3 system and optionally configure for testing"""
        fancy_print("Waiting for DDR3 initialization...", "info")
        
        # Wait for DDR3 to be ready (check with ? command)
        max_retries = 20
        for retry in range(max_retries):
            try:
                ...
                if response and 'R' in response:
                    fancy_print("DDR3 is ready!", "success")
                    break
                elif response and 'W' in response:
                    fancy_print(f"DDR3 still initializing... (attempt {retry+1}/{max_retries})", "warning")
                    time.sleep(1.0)
                else:
                    fancy_print(f"No response from DDR3 controller (attempt {retry+1}/{max_retries})", "warning")
                    time.sleep(0.5)
                    
            except Exception as e:
                fancy_print(f"Error checking DDR3 status: {e}", "error")
                time.sleep(0.5)
        ...
        # Only configure partial writes if enabled
        if USE_PARTIAL_WRITES:
            self.configure_partial_writes(True, PARTIAL_WRITE_CYCLES)
        else:
            fancy_print("Using normal write mode (8 full cycles)", "info")
    
    def configure_partial_writes(self, enable=True, cycles=4):
        """Configure partial write mode - only send command if enabling"""
        try:
            if enable:
                cmd = f"C1{cycles}\r"  # Enable partial write with specified cycles
                fancy_print(f"Enabling partial write mode ({cycles} cycles)", "info")
                self.ser.write(cmd.encode('ascii'))
                time.sleep(0.2)
                self.ser.reset_input_buffer()  # Clear any response
        except Exception as e:
            fancy_print(f"Warning: Could not configure partial writes: {e}", "warning")

    def write_cmd(self, addr, data):
        """Write command with improved timing"""
        cmd = f"W{addr:08X} {data}\r"
        self.ser.write(cmd.encode('ascii'))
        time.sleep(0.01)

    def read_cmd(self, addr):
        """Read command with improved error handling"""
        cmd = f"R{addr:08X}\r"
        self.ser.write(cmd.encode('ascii'))
        self.ser.flush()
        time.sleep(0.01)
            ...
            if self.ser.in_waiting:
                try:
                    ...
                    if line and len(line) >= 8:
                        # Extract the last 8 hex characters (the data)
                        hex_part = line[-8:].upper()
                        # Verify it's valid hex
                        int(hex_part, 16)  # This will raise ValueError if invalid
                        response = hex_part
                        break
    ...
    def debug_write_verify(self, addr, pattern, name="DEBUG"):
        """Debug a single write/verify cycle"""
        print(f"\n{Fore.YELLOW}DEBUG: Testing 0x{addr:08X} with {pattern}")
        
        # Write multiple times
        for i in range(3):
            self.write_cmd(addr, pattern)
            print(f"  Write {i+1}/3 sent")
        ...
        for i in range(3):
            ...
            status = "✓" if data == pattern else "✗"
            print(f"  Read {i+1}/3: {data} {status}")
            if data != pattern:
    ...
    def hamming_distance(self, hex1, hex2):
        ...
        try:
            ...
            return 32  # Maximum possible for 32-bit word
    ...
    def test_pattern_at_addresses(self, addresses, pattern_info, decay_time):
        ...
        # IMPORTANT: Wait for write completion
        time.sleep(0.1)
        
        # Verify phase with improved robustness
        verified = []
        ...
        try:
            for addr in addresses:
                ...
                for attempt in range(NVERIFY):
                    data = self.read_cmd(addr)
                    if data == pattern:
                        success = True
                        break
                    elif attempt < NVERIFY - 1:  # Don't sleep on last attempt
                        time.sleep(0.05)  # Wait between verify attempts
        ...
        if not verified:
            fancy_print(f"No addresses verified for {pattern_name} - skipping decay test", "warning")
            return []
        
        fancy_print(f"Verified {len(verified)}/{len(addresses)} addresses for {pattern_name}", "info")
        ...
        # Read back phase
        weak_found = []
        current_status = f"Reading back {pattern_name} after decay"
    ...
    def run_comprehensive_test(self):
        ...
        if USE_PARTIAL_WRITES:
            fancy_print(f"Partial writes: ENABLED ({PARTIAL_WRITE_CYCLES} cycles)", "warning")
        else:
            fancy_print(f"Partial writes: DISABLED (normal operation)", "info")
        
        # Optional: Run a quick debug test
        # self.debug_write_verify(0x10000000, "DEADBEEF", "Initial Test")
...
def main():
    ...
    if weak_cells:
        ...
        if not USE_PARTIAL_WRITES:
            print("5. Try enabling partial writes to stress cells differently")
