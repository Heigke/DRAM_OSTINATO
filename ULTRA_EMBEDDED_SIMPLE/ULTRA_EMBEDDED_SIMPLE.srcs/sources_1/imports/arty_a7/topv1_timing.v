        ...
        if (rx_stb) begin
            ...
            P_IDLE: begin
                ...
                if (rx_data == "?") begin
                    ...
                    cmd_is_write <= 0;
                    cmd_is_timing <= 0;
                end else if (rx_data=="T") begin  // NEW: Set timing
                    p_state <= P_GETTIM;
                    cmd_is_timing <= 1;
                end else if (rx_data=="t") begin  // NEW: Read timing
                    send_timing <= 1;
                end
            end
            //-----------------------------------------------------------
            ...
            //-----------------------------------------------------------
            P_GETTIM: begin  // NEW state
                if (is_hex) begin
                    shift <= {shift[59:0],hex2n(rx_data)};  
                    nib_cnt <= nib_cnt+1;
                end else if ((rx_data==8'h0d)||(rx_data==8'h0a)) begin
                    if (nib_cnt >= 8) begin  // Need at least 8 hex digits
...
always @(posedge clk_i) begin
    ...
    // Priority: status message over other responses
    if (send_status && !tx_busy) begin
        ...
        default: tx_cnt <= 15;
        endcase
    end else if (send_timing && !tx_busy) begin  // NEW: timing response
        case (tx_cnt)
        ...
        5:  begin tx_data <= 8'h0A; tx_stb <= 1; tx_cnt <= 15; send_timing <= 0; end
        default: tx_cnt <= 15;
        endcase
