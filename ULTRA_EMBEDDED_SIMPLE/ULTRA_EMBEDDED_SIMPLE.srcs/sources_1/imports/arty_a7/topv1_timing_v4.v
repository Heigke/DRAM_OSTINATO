...
always @(posedge clk_i) begin
    if (rst_i) begin
        ...
        // Start sending status when requested
        if (send_status_req && !sending_status && !sending_timing && !sending_axi_read) begin
            sending_status <= 1;
        end
        // Start sending timing when requested
        if (send_timing_req && !sending_status && !sending_timing && !sending_axi_read) begin
            sending_timing <= 1;
        end
        
        // Clear flags when transmission complete
        ...
        if (sending_axi_read && !tx_busy && tx_cnt == 15 && tx_busy_d) begin
            sending_axi_read <= 0;
        end
    end
end
