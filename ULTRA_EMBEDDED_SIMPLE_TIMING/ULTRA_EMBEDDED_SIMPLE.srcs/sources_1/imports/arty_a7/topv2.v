...
//  Fixed version with proper AXI handshaking and timing parameters
//
//  Commands:
//     WAAAAAAAA DDDDDDDD<CR>   → write 32-bit word
//     RAAAAAAAA<CR>            → read  32-bit word
//     ?<CR>                    → get status (R=ready, W=wait)
//
...
//=================================================================
//  Mini UART-to-AXI-Lite bridge (FIXED handshaking)
//=================================================================
...
always @(posedge clk_i) begin
    if (rst_i) begin
        ...
        // Parser ONLY sets wr_tag/rd_tag, never clears them
        // The AXI FSMs will clear them when transactions complete
...
//-----------------------------------------------------------------
// AXI Write FSM - FIXED: clears wr_tag only after BRESP
//-----------------------------------------------------------------
...
always @(posedge clk_i) begin
    if (rst_i) begin
        ...
        wlast_o <= 0;
        astate <= A_IDLE;
        wr_tag <= 0;  // FSM can clear, parser can set
    end else begin
        case (astate)
        ...
        A_B: if (bvalid_i) begin
            wr_tag <= 0;  // Clear AFTER response received
            astate <= A_IDLE;
        end
        endcase
    end
...
//-----------------------------------------------------------------
// AXI Read FSM - FIXED: clears rd_tag only after RLAST
//-----------------------------------------------------------------
...
always @(posedge clk_i) begin
    if (rst_i) begin
        arvalid_o <= 0;   
        rstate <= R_IDLE;
        rd_tag <= 0;  // FSM can clear, parser can set
    end else begin
        case (rstate)
        ...
        R_DATA: if (rvalid_i && rlast_i) begin
            rd_tag <= 0;  // Clear AFTER last data beat
            rstate <= R_IDLE;
        end
        endcase
    end
...
ddr3_axi #(
    .DDR_MHZ          (100),
    .DDR_WRITE_LATENCY(5),   // Increased from 4
    .DDR_READ_LATENCY (5)    // Increased from 4
) u_ddr (
    .clk_i (clk_w),
    .rst_i (rst_w),
...
//-----------------------------------------------------------------
// DDR3 PHY - ADJUSTED TIMING FOR ARTY
//-----------------------------------------------------------------
ddr3_dfi_phy #(
    .REFCLK_FREQUENCY  (200),
    .DQS_TAP_DELAY_INIT(24),    // Changed from 27 to 24 (typical for Arty)
    .DQ_TAP_DELAY_INIT (0),
    .TPHY_RDLAT        (6)      // Changed from 5 to 6
) u_phy (
    .clk_i         (clk_w),
    .rst_i         (rst_w),
