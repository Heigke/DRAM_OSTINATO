    ...
    def phase1_extreme_partial_charge_test(self):
        ...
        for burst_len in range(1, 9):
            ...
            if read_data:
                ...
                print(f"Burst {burst_len}: {charge_bar:<20} Read: {read_data} - {status} ({charge_estimate:.1%} charge)")
        ...
        self.burst_threshold = next((r['burst'] for r in results if r['errors'] < 8), 8)
        print(f"\n{Fore.GREEN}✓ Threshold burst length: {self.burst_threshold} cycles{Style.RESET_ALL}")
