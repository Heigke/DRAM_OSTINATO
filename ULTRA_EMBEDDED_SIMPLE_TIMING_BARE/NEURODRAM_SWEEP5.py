...
MEMORY_REGIONS = [
    {"name": "Bank 0", "start": 0x00000000, "end": 0x00100000, "step": 0x1000},
    {"name": "Bank 1", "start": 0x00100000, "end": 0x00200000, "step": 0x1000},
    {"name": "Bank 2", "start": 0x00200000, "end": 0x00300000, "step": 0x1000},
]
...
DECAY_TIMES = [0, 10, 30, 60, 120, 300]  # Up to 5 minutes
...
class NeuroCharacterizer:
    ...
    def initialize_ddr3(self):
        ...
        print(f"{Fore.CYAN}🔧 Resetting timing configuration to defaults...{Style.RESET_ALL}")
        self.configure_timing(twr=0, tras=0, burst_len=0, skip_refresh=0)
        time.sleep(0.2)
    ...
    def get_test_addresses(self, sample_size_per_region=100):
    ...
    def phase1_full_decay_analysis(self):
        ...
        # Get test addresses
        test_addresses = self.get_test_addresses(50)  # 50 per region = 150 total
        ...
        total_tests = len(test_addresses) * len(TEST_PATTERNS) * len(DECAY_TIMES)
        current_test = 0
        test_start_time = time.time()
        
        # Configure for full charge writes with refresh disabled
        ...
        for pattern_info in TEST_PATTERNS:
            ...
            for decay_time in DECAY_TIMES:
                ...
                for i, addr in enumerate(verified_addresses):
                    ...
                    self.progress_bar(i + 1, len(verified_addresses), 
                ...
                if decay_results:
                    ...
                    cells_with_errors = sum(1 for r in decay_results if r['errors'] > 0)
                    ...
                    if cells_with_errors > 0:
                        weak_cells_this_test = [r for r in decay_results if r['errors'] > 0]
                        print(f"  {Fore.MAGENTA}🎯 Found {len(weak_cells_this_test)} cells with errors:{Style.RESET_ALL}")
                        for cell in weak_cells_this_test[:3]:  # Show first 3
