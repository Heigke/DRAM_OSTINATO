...
# --- MODIFICATION: Added longer decay times for partial charge analysis ---
# Test longer decay times for partial charges to see how they behave over time.
PARTIAL_CHARGE_DECAY_TIMES = [0, 30, 120, 300, 600]
...
# --- MODIFICATION: Added constant for intensifying charge/discharge cycles ---
# Increase write repetitions to investigate charge build-up.
PARTIAL_CHARGE_WRITE_ITERATIONS = 15
...
class NeuroCharacterizer:
    ...
    def phase2_partial_charge_analysis(self):
        ...
        # --- MODIFICATION: Using the new extended decay times for the report ---
        print(f"⏱️  Test times: {PARTIAL_CHARGE_DECAY_TIMES}s\n")
        
        total_tests = len(test_cells) * len(PARTIAL_CHARGES) * len(PARTIAL_CHARGE_DECAY_TIMES)
        current_test = 0
        test_start_time = time.time()
        ...
        for charge_config in PARTIAL_CHARGES:
            ...
            # --- MODIFICATION: Looping over the new extended decay times ---
            for decay_time in PARTIAL_CHARGE_DECAY_TIMES:
                print(f"\n  Decay time: {decay_time}s")
                
                # --- MODIFICATION: Intensified cleaning cycle ---
                # First clean all cells more aggressively
                print(f"  Cleaning cells...", end='', flush=True)
                self.configure_timing(burst_len=8) # Use full burst for cleaning
                for addr in test_cells:
                    for _ in range(PARTIAL_CHARGE_WRITE_ITERATIONS):
                        self.write_cmd(addr, "00000000")
                print(f" {Fore.GREEN}✓{Style.RESET_ALL}")
                
                # --- MODIFICATION: Intensified partial charge writing ---
                # Write with partial charge more aggressively
                ...
                if charge_results:
                    ...
                    cells_responding = sum(1 for r in charge_results if r['charge_level'] > 0.01) # Respond if >1% charged
