...
// ---------------------------------------------------------------
// Parser  →  tiny command FIFO ( 1 slot each for R & W & T )
// ---------------------------------------------------------------
localparam P_IDLE   = 2'd0,
           P_GETADR = 2'd1,
           P_GETDAT = 2'd2,
           P_GETTIM = 2'd3;  // NEW state for timing data
...
reg  [31:0]  rd_addr;
reg          cmd_is_write;
reg          cmd_is_timing;  // NEW
reg  [31:0]  timing_data;    // NEW

// Feedback from AXI state machines
wire         wr_done;
wire         rd_done;

// NEW: Add timing command handling
reg          send_timing = 0;
...
always @(posedge clk_i) begin
  ...
  // This is the main operational logic when not in reset
  end else begin
    ...
    // Consolidated logic for send_timing
    if (send_timing && !tx_busy && tx_cnt == 5) begin
        send_timing <= 1'b0;
    end else if (rx_stb && p_state == P_IDLE && rx_data == "t") begin
        send_timing <= 1'b1;
    end
    ...
    if (rx_stb) begin
        case (p_state)
            ...
            // ... (rest of P_GETADR, P_GETDAT, P_GETTIM states) ...
            P_GETADR: begin
                if (is_hex) begin
...
// Modified to handle timing response
// In module mini_uart_axi, replace the whole response block with this:

always @(posedge clk_i) begin
    tx_busy_d <= tx_busy;
    tx_stb    <= 1'b0; // Default value: do not send

    // A single, prioritized FSM for the UART transmitter.
    // Highest priority: Status message for '?' command.
    if (send_status && !tx_busy) begin
        case (tx_cnt)
            15: begin tx_data <= init_done ? "R" : "W"; tx_stb <= 1; tx_cnt <= 14; end
            14: begin tx_data <= 8'h0A;                 tx_stb <= 1; tx_cnt <= 15; end // Done
            default: tx_cnt <= 15;
        endcase
    // Next priority: Timing message for 't' command.
    end else if (send_timing && !tx_busy) begin
        case (tx_cnt)
            ...
            5:  begin tx_data <= 8'h0A;                     tx_stb <= 1; tx_cnt <= 15; end // Done
            default: tx_cnt <= 15;
        endcase
    // Lowest priority: AXI read data response.
    // This now includes both starting the transmission and continuing it.
    end else if (!tx_busy) begin 
        if (rvalid_i) begin
            // Start of an AXI Read Data response
            tx_cnt  <= 0;
            tx_data <= n2h(rdata_i[31:28]);
            tx_stb  <= 1;
        // Continue sending if we just finished a previous byte and are not done
        end else if (tx_busy_d && tx_cnt < 8) begin
            tx_cnt <= tx_cnt + 1;
            // NOTE: We check the OLD value of tx_cnt to determine the NEXT byte to send.
            case (tx_cnt)
                0: begin tx_data <= n2h(rdata_i[27:24]); tx_stb <= 1; end
                1: begin tx_data <= n2h(rdata_i[23:20]); tx_stb <= 1; end
                2: begin tx_data <= n2h(rdata_i[19:16]); tx_stb <= 1; end
                3: begin tx_data <= n2h(rdata_i[15:12]); tx_stb <= 1; end
                4: begin tx_data <= n2h(rdata_i[11: 8]); tx_stb <= 1; end
                5: begin tx_data <= n2h(rdata_i[ 7: 4]); tx_stb <= 1; end
                6: begin tx_data <= n2h(rdata_i[ 3: 0]); tx_stb <= 1; end
                7: begin tx_data <= 8'h0a;               tx_stb <= 1; tx_cnt <= 15; end // Done, reset cnt
            endcase
        end
    end
end
