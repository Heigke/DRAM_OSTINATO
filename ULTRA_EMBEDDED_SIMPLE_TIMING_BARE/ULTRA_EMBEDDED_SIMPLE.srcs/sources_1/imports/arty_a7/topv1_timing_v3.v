...
reg          cmd_is_write;
reg          cmd_is_timing;
reg          sending_axi_read_response;  // For robust response handling

// Feedback from AXI state machines
wire         wr_done;
wire         rd_done;

// Timing command handling
reg          send_timing = 0;
...
always @(posedge clk_i) begin
    if (rst_i) begin
        ...
        timing_valid_o <= 0;
        send_timing <= 1'b0;
        sending_axi_read_response <= 1'b0;
    end else begin
        ...
        // Consolidated logic for send_timing flag (single driver)
        if (rx_stb && p_state == P_IDLE && rx_data == "t") begin
            send_timing <= 1'b1;
        end else if (send_timing && !tx_busy && tx_cnt == 5) begin
            send_timing <= 1'b0;
        end
        ...
        if (rx_stb) begin
            case (p_state)
                P_IDLE: begin
                    ...
                    if (rx_data == "?") begin
                        // Status command - handled in response block
...
//-----------------------------------------------------------------
// Status and echo response (v3 - Fixed timing response)
//-----------------------------------------------------------------
...
always @(posedge clk_i) begin
    if (rst_i) begin
        send_status <= 1'b0;
    end else if (rx_stb && rx_data == "?") begin
        send_status <= 1'b1;
    end else if (send_status && !tx_busy && tx_cnt == 14) begin
        send_status <= 1'b0;
    end
end

// This is the new, robust UART transmit state machine with fixed timing response
always @(posedge clk_i) begin
    tx_busy_d <= tx_busy;
    tx_stb    <= 1'b0; // Default value

    // Highest Priority: Status Message ('?')
    if (send_status && !tx_busy) begin
        case (tx_cnt)
            ...
            default: tx_cnt <= 15;
        endcase
    // Second Priority: Timing Message ('t')
        ...
        case (tx_cnt)
            ...
            5:  begin 
                ...
                tx_stb <= 1; 
                tx_cnt <= 15; 
                // send_timing is now cleared in the parser block to avoid multiple drivers
            end
            default: tx_cnt <= 15;
        endcase
    // Third Priority: AXI Read Data Response (if already in progress)
    end else if (sending_axi_read_response && !tx_busy) begin
        // Continue sending subsequent bytes of the AXI read data
        tx_cnt <= tx_cnt + 1;
        case (tx_cnt)
            ...
            default: sending_axi_read_response <= 1'b0;
        endcase
    // Lowest Priority: Start a new AXI Read Data Response
    end else if (rvalid_i && !tx_busy) begin
        sending_axi_read_response <= 1'b1;
        tx_cnt  <= 0;
