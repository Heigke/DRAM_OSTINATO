...
class NeuromorphicDRAMAnalyzer:
    ...
    def quick_weak_cell_sweep(self):
        ...
        # Calculate total scans for accurate progress
        total_scans = 0
        for region in MEMORY_REGIONS:
            region_addrs = list(range(region['start'], region['end'], region['step']))
            sample_size = max(10, len(region_addrs) // 10)
            total_scans += min(sample_size, len(region_addrs))
        
        fancy_print(f"Quick sweep will test {total_scans} samples across {len(MEMORY_REGIONS)} regions", "info")
        ...
        for region_idx, region in enumerate(MEMORY_REGIONS):
            ...
            fancy_print(f"Scanning {region['name']}: {len(sampled_addrs)} samples", "info")
            
            # Write phase with progress
            write_addresses = []
            print(f"\n  {Fore.YELLOW}Writing test pattern...{Style.RESET_ALL}")
            for i, addr in enumerate(sampled_addrs):
                for _ in range(NWRITES):
                ...
                if i % 5 == 0 or i == len(sampled_addrs) - 1:
                    progress = (i + 1) / len(sampled_addrs)
                    bar_length = 30
                    filled = int(bar_length * progress)
                    bar = f"{Fore.GREEN}{'█' * filled}{Fore.WHITE}{'░' * (bar_length - filled)}"
                    print(f"\r  Writing: [{bar}] {progress*100:.0f}% ({i+1}/{len(sampled_addrs)})", end='', flush=True)
            
            print()  # New line after write progress
            
            # Decay wait with animated timer
            if write_addresses:
                print(f"\n  {Fore.CYAN}Waiting for decay...{Style.RESET_ALL}")
                decay_time = 30
                start_wait = time.time()
                ...
                print(f"\n  {Fore.BLUE}Reading back and analyzing...{Style.RESET_ALL}")
                for i, addr in enumerate(write_addresses):
                    ...
                    if i % 5 == 0 or i == len(write_addresses) - 1:
                        progress = (i + 1) / len(write_addresses)
                        bar_length = 30
                        filled = int(bar_length * progress)
                        bar = f"{Fore.BLUE}{'█' * filled}{Fore.WHITE}{'░' * (bar_length - filled)}"
                        print(f"\r  Reading: [{bar}] {progress*100:.0f}% ({i+1}/{len(write_addresses)}) - Found: {len(weak_candidates)}", end='', flush=True)
                
                print()  # New line after read progress
            
            # Overall progress
            overall_progress = (scan_count / total_scans)
            print(f"\n  {Fore.MAGENTA}Overall Progress: {overall_progress*100:.1f}% - Weak cells found: {len(weak_candidates)}{Style.RESET_ALL}")
