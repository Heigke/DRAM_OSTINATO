...
class NeuromorphicDRAMAnalyzer:
    ...
    def quick_weak_cell_sweep(self):
        ...
        fancy_print(f"Scanning ALL {total_addresses} addresses for decay characteristics", "info")
        ...
        # Test ALL addresses like the old script
        total_scans = total_addresses
        
        fancy_print(f"Full sweep will test ALL {total_scans} addresses across {len(MEMORY_REGIONS)} regions", "info")
        # Test with multiple decay times like the old script
        decay_times_to_test = [30, 60, 120]  # Test multiple decay times
        
        for region_idx, region in enumerate(MEMORY_REGIONS):
            region_addrs = list(range(region['start'], region['end'], region['step']))
            
            # Test ALL addresses, not just a sample
            fancy_print(f"Scanning {region['name']}: {len(region_addrs)} addresses", "info")
            
            for decay_time in decay_times_to_test:
                fancy_print(f"Testing with {decay_time}s decay time", "info")
                
                # Write phase with progress
                write_addresses = []
                print(f"\n  {Fore.YELLOW}Writing test pattern to all addresses...{Style.RESET_ALL}")
                
                for i, addr in enumerate(region_addrs):
                    for _ in range(10):  # Multiple writes like old script
                    ...
                    if i % 20 == 0 or i == len(region_addrs) - 1:
                        progress = (i + 1) / len(region_addrs)
                        bar_length = 30
                        filled = int(bar_length * progress)
                        bar = f"{Fore.GREEN}{'█' * filled}{Fore.WHITE}{'░' * (bar_length - filled)}"
                        print(f"\r  Writing: [{bar}] {progress*100:.0f}% ({i+1}/{len(region_addrs)})", end='', flush=True)
                
                print()  # New line after write progress
                
                # Verify writes
                print(f"\n  {Fore.BLUE}Verifying writes...{Style.RESET_ALL}")
                for i, addr in enumerate(region_addrs):
                    data = self.read_cmd(addr)
                    if data == test_pattern['pattern']:
                        write_addresses.append(addr)
                    
                    if i % 20 == 0 or i == len(region_addrs) - 1:
                        progress = (i + 1) / len(region_addrs)
                        bar_length = 30
                        filled = int(bar_length * progress)
                        bar = f"{Fore.BLUE}{'█' * filled}{Fore.WHITE}{'░' * (bar_length - filled)}"
                        print(f"\r  Verifying: [{bar}] {progress*100:.0f}% - Verified: {len(write_addresses)}", end='', flush=True)
                
                print()
                
                if not write_addresses:
                    fancy_print(f"No addresses verified, skipping decay test", "warning")
                    continue
                
                fancy_print(f"Verified {len(write_addresses)}/{len(region_addrs)} addresses", "info")
                
                # Decay wait with animated timer
                print(f"\n  {Fore.CYAN}Waiting {decay_time}s for decay...{Style.RESET_ALL}")
                start_wait = time.time()
                ...
                print()  # New line after decay timer
                
                # Read phase with progress
                print(f"\n  {Fore.MAGENTA}Reading back and analyzing...{Style.RESET_ALL}")
                found_this_run = 0
                    ...
                    if readback and readback != test_pattern['pattern']:
                        ...
                        if hamming > 0:
                            weak_candidates.append({
                                ...
                                'expected': test_pattern['pattern'],
                                'readback': readback
                            })
                            found_this_run += 1
                    
                    # Update read progress
                    if i % 20 == 0 or i == len(write_addresses) - 1:
                        progress = (i + 1) / len(write_addresses)
                        bar_length = 30
                        filled = int(bar_length * progress)
                        bar = f"{Fore.MAGENTA}{'█' * filled}{Fore.WHITE}{'░' * (bar_length - filled)}"
                        print(f"\r  Reading: [{bar}] {progress*100:.0f}% ({i+1}/{len(write_addresses)}) - Found this run: {found_this_run}", end='', flush=True)
                
                print()  # New line after read progress
                
                if found_this_run > 0:
                    fancy_print(f"Found {found_this_run} weak cells with {decay_time}s decay!", "found")
                    # Show some examples
                    for cell in weak_candidates[-min(3, found_this_run):]:
                        print(f"    → 0x{cell['addr']:08X}: {cell['expected']} → {cell['readback']} ({cell['initial_decay']} bits)")
            
            # Overall progress
            overall_progress = ((region_idx + 1) / len(MEMORY_REGIONS))
            print(f"\n  {Fore.CYAN}Region Progress: {overall_progress*100:.1f}% - Total weak cells found: {len(weak_candidates)}{Style.RESET_ALL}")
            print(f"  {Fore.WHITE}{'─' * 60}{Style.RESET_ALL}\n")
        
        clear_line()
        fancy_print(f"Quick sweep complete! Found {len(weak_candidates)} candidates for detailed analysis", "found")
        
        # If no weak cells found, suggest solutions
        if len(weak_candidates) == 0:
            print(f"\n{Fore.YELLOW}No weak cells found. Suggestions:")
            print("  1. Increase DRAM temperature to 70-85°C")
            print("  2. Try longer decay times")
            print("  3. Check DDR3 initialization")
            print(f"  4. Enable partial writes for different stress patterns{Style.RESET_ALL}\n")
