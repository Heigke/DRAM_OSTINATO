...
# Quick partial write configs for initial screening - THIS WILL BE DYNAMICALLY SET
...
class NeuromorphicDRAMAnalyzer:
    def __init__(self, serial_port):
        ...
        # **MODIFIED**: Use a fallback config that can be dynamically updated
    ...
    # **NEW FUNCTION**: More robust analysis of partial charge sensitivity
    def analyze_quick_sensitivity(self, results):
        """
        Analyzes results from a quick partial charge test for a sensitivity gradient.
        A cell is sensitive if errors monotonically decrease as charge (cycles) increases.
        """
        if len(results) < 3:
            return False

        try:
            hammings = {r['cycles']: r['hamming'] for r in results}
            cycles = sorted(hammings.keys())

            h1 = hammings[cycles[0]] # Errors at sub-threshold charge
            h2 = hammings[cycles[1]] # Errors at threshold charge
            h3 = hammings[cycles[2]] # Errors at super-threshold charge

            # Condition 1: The lowest charge must result in a bad write (high error).
            # This confirms we are starting from a "weakly written" or "unwritten" state.
            weak_write_fails = h1 > 10 

            # Condition 2: The highest charge must result in a good write (low error).
            # This confirms the cell is actually writable.
            strong_write_works = h3 < 8

            # Condition 3: The error rate must decrease as charge increases.
            # We look for a clear downward trend (monotonic decrease).
            is_monotonic = h1 > h2 >= h3

            # A cell is sensitive if it starts bad, gets good, and shows a trend in between.
            if weak_write_fails and strong_write_works and is_monotonic:
                return True
    ...
    # **MODIFIED FUNCTION**: Uses dynamic configs and the new analysis function.
    def quick_partial_charge_test(self, addr, pattern_info):
        """Quick test to see if a cell is sensitive to partial charges using dynamic configs."""
        pattern = pattern_info['pattern']
        results = []
        
        # Test with 3 charge levels dynamically chosen to bracket the write threshold
        for config in self.quick_partial_configs:
            cycles = config['cycles']
            
            # Configure partial writes
            self.configure_timing(burst_len=cycles)
            time.sleep(0.05) # Shorter sleep for quick test
            ...
            # Quick decay test (just 5 seconds to see immediate stability)
            time.sleep(5)
        ...
        # **MODIFIED**: Analyze results using the new, more robust function
        is_sensitive = self.analyze_quick_sensitivity(results)
        ...
        try:
            for addr_idx, addr in enumerate(all_addresses):
                ...
                if is_sensitive:
                    ...
                    # Print results clearly for found cells
                    hammings = [r['hamming'] for r in results]
                    cycles = [r['cycles'] for r in results]
                    clear_line()
                    fancy_print(f"Found candidate #{found} at 0x{addr:08X}", "found")
                    print(f"  {Fore.GREEN}└─ Gradient Found! Cycles: {cycles} -> Errors: {hammings}{Style.RESET_ALL}")

                tested += 1
                
                # Update status periodically
                if tested % 50 == 0:
                    current_status = f"Tested {tested}/{len(all_addresses)}, found {found} sensitive cells"
    ...
    # **MODIFIED FUNCTION**: Includes dynamic configuration step
    def run_comprehensive_analysis(self):
        ...
        # **NEW**: Dynamically configure the quick partial test based on gradient results
        ...
        # Phase 1: Quick partial charge sensitivity sweep (now using dynamic configs)
        partial_sensitive_cells = self.partial_charge_sweep()
        
        if not partial_sensitive_cells:
            fancy_print("No partial charge sensitive cells found. Try adjusting temperature or voltage.", "warning")
            fancy_print("Aborting detailed analysis.", "error")
        ...
        user_input = input(f"\n{Fore.YELLOW}Run optional decay analysis? This may take ~30 minutes. (y/N): {Style.RESET_ALL}")
...
def main():
   ...
   print(f"║  2. Dynamic Partial Sweep - Find cells with analog behavior at threshold.    ║")
   print(f"║  3. Detailed characterization - Profile sensitivity of best candidates.      ║")
   print(f"║                                                                              ║")
   print(f"║  This adaptive approach is faster and more accurate!                         ║")
   print(f"╚══════════════════════════════════════════════════════════════════════════════╝{Style.RESET_ALL}\n")
   ...
   try:
       ...
       if not cell_profiles:
           fancy_print("No cells were fully profiled. Check logs for errors.", "warning")
           return
