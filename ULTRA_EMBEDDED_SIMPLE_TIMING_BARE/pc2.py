...
class NeuromorphicDRAMAnalyzer:
    ...
    # ... [All other class methods like burst_length_gradient_test, extreme_timing_sweep, etc., remain unchanged] ...
    ...
    # **MODIFIED**: This is the new, more selective test
    def quick_partial_charge_test(self, addr, pattern_info):
        """
        Quick test to see if a cell is sensitive to partial charges AND shows interesting decay.
        It performs an immediate read and a read after a delay to check for stability.
        """
        pattern = pattern_info['pattern']
        results = []
        DECAY_WAIT_SECONDS = 60  # Wait 10s to see if the cell state degrades
        ...
        for config in self.quick_partial_configs:
            ...
            # 1. Immediate Read
            time.sleep(0.05)
            data_imm = self.read_cmd(addr)
            hamming_imm = self.hamming_distance(pattern, data_imm)

            # 2. Decayed Read
            time.sleep(DECAY_WAIT_SECONDS)
            data_decay = self.read_cmd(addr)
            hamming_decay = self.hamming_distance(pattern, data_decay)

            results.append({
                'cycles': cycles,
                'hamming_imm': hamming_imm,
                'hamming_decay': hamming_decay,
            })
        
        self.configure_timing(burst_len=0) # Reset timing
        
        is_sensitive = self.analyze_quick_sensitivity_with_decay(results)
        
        return is_sensitive, results

    # **MODIFIED**: This is the new, more selective analysis function
    def analyze_quick_sensitivity_with_decay(self, results):
        """
        Analyzes results for both a charge gradient and anomalous decay.
        An interesting cell shows the expected charge gradient but decays faster than normal.
        """
        if len(results) < 3:
            return False

        try:
            imm_hammings = {r['cycles']: r['hamming_imm'] for r in results}
            decay_hammings = {r['cycles']: r['hamming_decay'] for r in results}
            cycles = sorted(imm_hammings.keys())

            h_imm_c1 = imm_hammings[cycles[0]] # Errors at sub-threshold charge (e.g., burst=2)
            h_imm_c2 = imm_hammings[cycles[1]] # Errors at threshold charge (e.g., burst=3)
            h_imm_c3 = imm_hammings[cycles[2]] # Errors at super-threshold charge (e.g., burst=4)

            # Condition 1: Check for the standard immediate write gradient.
            # This establishes the baseline behavior you observed (e.g., errors go from 17 to 0).
            standard_gradient = (h_imm_c1 > 10) and (h_imm_c2 < 4) and (h_imm_c3 < 4)

            if not standard_gradient:
                # If it doesn't even show the normal write behavior, it's not the type
                # of candidate we're looking for with this test. It might be a stuck bit.
                return False

            # Condition 2: Check for abnormal decay.
            # The key insight: A "normal" cell is stable, so its errors shouldn't increase.
            # A "neuromorphic" cell should be unstable and show more errors after the delay.
            h_decay_c2 = decay_hammings[cycles[1]] # Get errors for the threshold write after decay

            # We define abnormal decay as the cell state degrading significantly.
            # A perfectly stable cell would have h_decay_c2 == h_imm_c2 (e.g., 0 == 0).
            # If h_decay_c2 has increased by more than 2 bits, it's interesting.
            decayed_significantly = (h_decay_c2 - h_imm_c2) > 2

            # An ideal candidate shows a normal write gradient BUT decays significantly.
            return decayed_significantly

        except (KeyError, IndexError):
            return False
    
    def partial_charge_sweep(self):
        """Quick sweep to find cells sensitive to partial charges and fast decay."""
        fancy_print("PHASE 1: Quick Neuromorphic Candidate Sweep (Charge + Decay)", "charge")
        
        partial_sensitive_candidates = []
        total_addresses = sum(len(range(r['start'], r['end'], r['step'])) for r in MEMORY_REGIONS)
        
        fancy_print(f"Testing {total_addresses} addresses for anomalous decay properties", "info")
        ...
        current_status = f"Scanning {len(all_addresses)} addresses for leaky cells"
        ...
        try:
            for addr in all_addresses:
                ...
                if is_sensitive:
                    clear_line()
                    imm_errors = [r['hamming_imm'] for r in results]
                    decay_errors = [r['hamming_decay'] for r in results]
                    fancy_print(f"Found Leaky Candidate #{found+1} at 0x{addr:08X}", "found")
                    print(f"  {Fore.YELLOW}└─ Immediate errors: {imm_errors}")
                    print(f"  {Fore.RED}   └─ Decayed errors:   {decay_errors} (Degradation detected!){Style.RESET_ALL}")
                ...
                tested += 1
                if tested % 20 == 0:
                    current_status = f"Scanned {tested}/{len(all_addresses)}, found {found} leaky cells"
        ...
        clear_line()
        fancy_print(f"Neuromorphic candidate sweep complete! Found {len(partial_sensitive_candidates)} leaky cells.", "found")
        
        return partial_sensitive_candidates

    # ... [All other class methods like detailed_partial_charge_characterization, run_comprehensive_analysis, etc., remain unchanged] ...
