...
// Fixed version with proper burst timing and debugging
module ddr3_decay_test_top (
    ...
    logic [4:0]  read_delay_cnt_q;  // Counter for read latency
    ...
    always_ff @(posedge clk_phy_sys or posedge sys_rst_fsm_phy) begin
        if (sys_rst_fsm_phy) begin
            ...
            if (current_state_q == S_WRITE_CMD) begin
                write_delay_cnt_q <= 4'd0;
            end else if (current_state_q == S_WRITE_DATA && write_delay_cnt_q < TPHY_WRLAT_C) begin
            ...
            if (current_state_q == S_READ_CMD) begin
                read_delay_cnt_q <= 5'd0;
            end else if (current_state_q == S_READ_DATA && read_delay_cnt_q < (TPHY_RDLAT_C + CAS_LATENCY_C)) begin
                read_delay_cnt_q <= read_delay_cnt_q + 1;
            end
            
            // Burst counter
            if (current_state_q == S_WRITE_CMD || current_state_q == S_READ_CMD) begin
                burst_cnt_q <= 3'd0;
            end else if ((current_state_q == S_WRITE_DATA && write_delay_cnt_q >= TPHY_WRLAT_C && burst_cnt_q < 3'd3) ||
                         (current_state_q == S_READ_DATA && read_delay_cnt_q >= (TPHY_RDLAT_C + CAS_LATENCY_C) && burst_cnt_q < 3'd3)) begin
                burst_cnt_q <= burst_cnt_q + 1;
            end
            
            // Read data capture - capture on consecutive cycles after latency
            if (current_state_q == S_READ_DATA && read_delay_cnt_q >= (TPHY_RDLAT_C + CAS_LATENCY_C)) begin
                case (burst_cnt_q)
    ...
    always_comb begin
        ...
        case (current_state_q)
            ...
            S_WRITE_DATA: begin
                ...
                // Exit after 4 beats
                if (burst_cnt_q == 3'd3 && write_delay_cnt_q >= TPHY_WRLAT_C) begin
            ...
            // Unified read burst state
            S_READ_DATA: begin
                dfi_rddata_en_s = 1'b1;  // Keep read enable high
                
                // Exit after capturing 4 beats
                if (burst_cnt_q == 3'd3 && read_delay_cnt_q >= (TPHY_RDLAT_C + CAS_LATENCY_C)) begin
    ...
    // Enhanced ILA signals for debugging with command pulses
    logic [5:0]  ila_state;
    ...
    ila_0 u_ila (
        ...
        .probe12(init_done_q),                 
        .probe13({ila_data_match, ila_write_delay_cnt, ila_burst_cnt}), 
        .probe14({3'd0, ila_read_delay_cnt}),
        .probe15({ila_read_delay_cnt[3:0], ila_write_cmd_pulse, ila_read_cmd_pulse, ila_rddata_valid, ila_wrdata_en})
    );
