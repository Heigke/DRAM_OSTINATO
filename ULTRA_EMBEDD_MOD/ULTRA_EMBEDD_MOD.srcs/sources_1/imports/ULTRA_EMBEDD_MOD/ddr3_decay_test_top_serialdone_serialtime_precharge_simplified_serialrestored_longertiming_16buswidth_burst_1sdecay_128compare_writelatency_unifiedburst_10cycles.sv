...
// Fixed version with corrected burst capture timing
module ddr3_decay_test_top (
    ...
    logic        read_capture_active_q; // Flag to indicate when to capture read data
    ...
    always_ff @(posedge clk_phy_sys or posedge sys_rst_fsm_phy) begin
        if (sys_rst_fsm_phy) begin
            ...
            if (current_state_q == S_WRITE_CMD) begin
                ...
                end else if (burst_cnt_q < 3'd3) begin
                    burst_cnt_q <= burst_cnt_q + 1;
                end
            end
            
            // Read delay counter and burst control
            if (current_state_q == S_READ_CMD) begin
                read_delay_cnt_q <= 5'd0;
                burst_cnt_q <= 3'd0;
                read_capture_active_q <= 1'b0;
            end else if (current_state_q == S_READ_DATA) begin
                if (read_delay_cnt_q < (TPHY_RDLAT_C + CAS_LATENCY_C - 1)) begin
                    read_delay_cnt_q <= read_delay_cnt_q + 1;
                end else begin
                    // Start capturing when latency expires
                    read_capture_active_q <= 1'b1;
                    if (read_capture_active_q && burst_cnt_q < 3'd4) begin
                        burst_cnt_q <= burst_cnt_q + 1;
                    end
                end
            end
            ...
            if (current_state_q == S_READ_DATA && read_capture_active_q) begin
                case (burst_cnt_q)
    ...
    always_comb begin
        ...
        case (current_state_q)
            ...
            S_READ_DATA: begin
                ...
                if (burst_cnt_q >= 3'd4 && read_delay_cnt_q >= (TPHY_RDLAT_C + CAS_LATENCY_C - 1)) begin
    ...
    always_comb begin
        ...
        ila_capture_active = read_capture_active_q;
    end
    ...
    ila_0 u_ila (
        ...
        .probe15({ila_capture_active, ila_burst_cnt, ila_write_cmd_pulse, ila_read_cmd_pulse, ila_rddata_valid, ila_wrdata_en})
    );
