...
module ddr3_decay_test_top (
    ...
    // --- DFI PHY Instance ---
    // FIX #1: Update read latency from 4 to 5
    localparam TPHY_RDLAT_C  = 5;  // Changed from 4 to 5
    ...
    always_ff @(posedge clk_phy_sys or posedge sys_rst_fsm_phy) begin
        if (sys_rst_fsm_phy) begin
            ...
            if (current_state_q != S_READ_DATA) begin
                ...
                // FIX #2: Capture data with unswap16 to correct half-word ordering
                case (rd_beat_cnt_q)
    ...
    always_comb begin
        ...
        case (current_state_q)
            ...
            S_READ_CMD: begin
                ...
                dfi_bank_s = TEST_BANK;
                dfi_address_s = {5'd0, TEST_COL};
                // FIX #3: Only pulse rddata_en for one cycle
            ...
            // Read burst state
            S_READ_DATA: begin
                // FIX #3: Do NOT assert rddata_en again
                dfi_rddata_en_s = 1'b0;
