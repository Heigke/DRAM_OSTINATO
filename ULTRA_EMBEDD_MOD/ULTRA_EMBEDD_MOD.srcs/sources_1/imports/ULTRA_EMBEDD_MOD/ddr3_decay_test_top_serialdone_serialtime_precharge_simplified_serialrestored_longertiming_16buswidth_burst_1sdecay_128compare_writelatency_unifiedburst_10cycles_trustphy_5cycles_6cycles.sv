...
// Fixed version with correct read latency and 16-bit half-word ordering
...
// ❶  asserted the same cycle we drive the PHY
logic        ila_rddata_en;        // 1-cycle pulse window → PHY  
// ❷  down-counter for the fixed CAS latency period
logic [2:0]  ila_rd_lat_cnt;       // counts 5-4-3-2-1-0  
// ❸  down-counter for the 4-beat BL8 read window
logic [2:0]  ila_rddata_en_cnt;    // counts 4-3-2-1-0  
    // MMCM instance
    clk_wiz_0 u_clk_wiz (
    ...
    // --- DFI PHY Instance ---
    // FIX #1: Update read latency to match measured 6 cycles
    localparam TPHY_RDLAT_C  = 6;  // Changed from 4 to 6 (measured in ILA)
    ...
    logic [2:0]  rddata_en_cnt_q;   // Counter for rddata_en assertion
    logic [2:0]  rd_lat_cnt_q;      // NEW: Latency counter for read timing
    ...
    // FIX #2: Add 16-bit half-word swap functions
    ...
    always_ff @(posedge clk_phy_sys or posedge sys_rst_fsm_phy) begin
        if (sys_rst_fsm_phy) begin
            ...
            // FIX: Proper read latency and enable management
            if (current_state_q == S_READ_CMD) begin
                rd_lat_cnt_q <= TPHY_RDLAT_C - 1;  // 5 counts down to 0
                rddata_en_cnt_q <= 3'd0;           // Wait for latency first
            end else if (rd_lat_cnt_q != 0) begin
                rd_lat_cnt_q <= rd_lat_cnt_q - 1;
            end else if (rddata_en_cnt_q != 0) begin
                rddata_en_cnt_q <= rddata_en_cnt_q - 1;
            end
            
            // Start the four-beat window after latency expires
            if (rd_lat_cnt_q == 0 && rddata_en_cnt_q == 0 && 
                current_state_q == S_READ_DATA) begin
                rddata_en_cnt_q <= 3'd4;  // Enable for 4 beats
            end
    ...
    always_comb begin
        ...
        // FIX #2: Use counter to drive rddata_en
        ...
        case (current_state_q)
            ...
            S_WRITE_DATA: begin
                ...
                // FIX #4: Enable write data only for the exact burst beats
                if (write_delay_cnt_q >= TPHY_WRLAT_C && write_burst_cnt_q < 3'd4) begin
                    dfi_wrdata_en_s = 1'b1;
                    dfi_wrdata_mask_s = 4'h0;
                    
                    // FIX #2: Apply swap16 to correct half-word ordering
            ...
            S_READ_CMD: begin
                ...
                dfi_bank_s = TEST_BANK;
                dfi_address_s = {5'd0, TEST_COL};
                // FIX #2: rddata_en is now handled by the counter
                next_state_s = S_READ_DATA;
            end
            
            // Read burst state
            S_READ_DATA: begin
                // FIX #2: rddata_en is handled by counter, no explicit assignment needed
    ...
    // ILA instance with simplified probes
    ila_0 u_ila (
        .clk(clk_phy_sys),
        .probe0(ila_state),                    
        ...
        .probe14({2'd0, ila_rd_beat_cnt, ila_rddata_valid, ila_write_cmd_pulse, ila_read_cmd_pulse, ila_read_burst_done}),
