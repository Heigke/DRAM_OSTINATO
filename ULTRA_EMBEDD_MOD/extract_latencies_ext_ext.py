...
# Columns to use (add more as needed)
KEY_FIELDS = {
...
# Command types (update as needed for your FSM)
READ_CMD_TYPE = "5"
WRITE_CMD_TYPE = "4"
...
# --- For burst grouping ---
grouped_read_bursts = []
grouped_write_bursts = []
current_read_burst = []
current_write_burst = []

# --- For additional tracking ---
extra_read_beats = []
used_read_samples = set()
used_write_samples = set()
...
# --- Find extra read/write beats not grouped ---
for beat in raw_read_beats:
    if beat["sample"] not in used_read_samples:
        extra_read_beats.append(beat)
for beat in raw_write_beats:
    if beat["sample"] not in used_write_samples:
        extra_write_beats.append(beat)

# --- Output for model feeding and debugging ---
print("="*80)
