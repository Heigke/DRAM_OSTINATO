...
class DDR3DecayVisualizer:
    ...
    def setup_plots(self):
        ...
        # Heatmap for error rates vs address/time
    ...
    def parse_uart_line(self, line):
        ...
        # Format: "DT:xxxxx,ADDR:xxxx,MEAS:x,ERR:xx,PASS:x,RD:xxxxxxxx"
    ...
    def update_plots(self, frame):
        ...
        while not self.data_queue.empty() and processed < 100:
            try:
                ...
                with self.data_lock:
                    key = (data['address'], data['decay_time_ms'])
    ...
    def update_heatmap(self):
        ...
        with self.data_lock:
            ...
            # Get unique addresses and decay times
            ...
            for i, addr in enumerate(addresses):
                for j, time in enumerate(decay_times):
                    if time in self.decay_data[addr]:
                        ...
                        # Average error rate as percentage of 128 bits
            ...
            self.ax_heatmap.set_xticklabels([f'{t:.1f}' for t in decay_times][::max(1, len(decay_times)//10)], 
                                           rotation=45)
            self.ax_heatmap.set_yticks(np.arange(len(addresses))[::max(1, len(addresses)//20)])
            self.ax_heatmap.set_yticklabels([f'0x{addr:04X}' for addr in addresses][::max(1, len(addresses)//20)])
            ...
            # Add colorbar
            #if hasattr(self, 'heatmap_cb'):
            #    self.heatmap_cb.remove()
            if hasattr(self, 'heatmap_cb') and self.heatmap_cb is not None:
              self.heatmap_cb.remove()
    ...
    def update_decay_curve(self):
        ...
        with self.data_lock:
            ...
            # Calculate average error rate for each decay time
            decay_times = []
            avg_errors = []
            std_errors = []
            ...
            for time in all_times:
                ...
                if all_errors:
                    ...
                    # Convert to percentage of 128 bits
    ...
    def save_analysis(self):
        ...
        with open(summary_filename, 'w') as f:
            ...
            with self.data_lock:
                ...
                # Write detailed statistics for each decay time
                all_times = sorted(set(time for addr_data in self.decay_data.values() 
                                     for time in addr_data.keys()))
